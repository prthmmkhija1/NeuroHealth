...
CONDITION_SCHEMA = {
    ...
    "anxiety_disorder": {
        ...
        "red_flags": [
            "panic attacks",
            "avoidance leading to disability",
...
def get_conditions_for_symptom(symptom_key):
    ...
    return sorted(
        matches,
        key=lambda x: urgency_order.get(x.get("default_urgency", "ROUTINE"), 99),
    )
...
def check_urgency_rules(symptom_keys, user_text_lower):
    ...
    for rule in URGENCY_RULES:
        ...
        modifier_match = any(
            m.lower() in user_text_lower for m in rule["trigger_modifiers"]
        )
...
def export_schema_json():
    ...
    schema = {
        ...
        "urgency_rules": [{k: v for k, v in rule.items()} for rule in URGENCY_RULES],
...
if __name__ == "__main__":
    ...
    for cond in get_conditions_for_symptom("chest_pain"):
        print(
            f"  {cond['display_name']} → {cond['default_urgency']} → {cond['specialist']}"
        )
